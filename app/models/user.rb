class User < ActiveRecord::Base
  ...
  has_many :following, through: :follow_relationships_out, source: :user
  ...
  def current_events
    ...
    friend_events = Event.current.joins(:participants).where('users.id = ?', id).where('public = 0').order(start_time: :desc)
